  ...
  return (
    <TaskProvider>
      <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-800 selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <header className="lg:hidden fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-4 h-14 shadow-sm bg-[#82B29A] print:hidden">
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 w-full bg-white border-t border-slate-200 flex justify-around items-stretch pb-safe z-50 shadow-lg print:hidden">
          <Link href="/overview" className={`flex-1 pt-2 pb-3 px-2 flex flex-col items-center gap-1 transition-colors relative ${pathname === '/overview' ? 'text-[#82B29A]' : 'text-slate-400'}`}>
            <span className={`absolute top-0 left-3 right-3 h-0.5 rounded-b-full transition-colors ${pathname === '/overview' ? 'bg-[#82B29A]' : 'bg-transparent'}`}></span>
          ...
          <Link href="/wfo" className={`flex-1 pt-2 pb-3 px-2 flex flex-col items-center gap-1 transition-colors relative ${isWfoActive ? 'text-[#82B29A]' : 'text-slate-400'}`}>
            <span className={`absolute top-0 left-3 right-3 h-0.5 rounded-b-full transition-colors ${isWfoActive ? 'bg-[#82B29A]' : 'bg-transparent'}`}></span>
          ...
          <Link href="/wfa" className={`flex-1 pt-2 pb-3 px-2 flex flex-col items-center gap-1 transition-colors relative ${isWfaActive ? 'text-[#82B29A]' : 'text-slate-400'}`}>
            <span className={`absolute top-0 left-3 right-3 h-0.5 rounded-b-full transition-colors ${isWfaActive ? 'bg-[#82B29A]' : 'bg-transparent'}`}></span>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
