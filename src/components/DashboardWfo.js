...
export default function DashboardWfo({ tasks, refreshData, updateTaskLocal, deleteTaskLocal, onOpenForm }) {
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-end mb-6 pb-6 border-b border-slate-200/80">
          ...
          <div className="w-full md:w-auto ml-auto mt-2 md:mt-0 flex flex-row gap-2">
            ...
            <button onClick={() => generatePdfWfo(filteredTasks, filterBulan)} className="w-full bg-[#E5B3BB] hover:bg-[#D49BA5] text-white font-bold py-2.5 px-5 rounded-md transition-all shadow-md flex justify-center items-center gap-2 text-sm sm:text-base whitespace-nowrap">
                    ...
                    <div key={item.id || `${groupIndex}-${index}`} className="bg-white border border-slate-200 rounded-xl p-3.5 mb-2.5 shadow-sm active:bg-slate-50">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex-1 min-w-0">
                          {index === 0 && (
                            <span className="inline-flex text-[10px] font-bold text-white px-2 py-0.5 rounded-full mr-1" style={{ backgroundColor: "#82B29A" }}>
                              {group.tanggal}
                            </span>
                          )}
