...
export default function Overview({ tasks }) {
  ...
  const COLORS = ['#82B29A', '#F4A261']; // Kemenag Teal, WFA Yellow
    ...
    <div className="w-full relative">
      ...
      <div className="mb-6 lg:mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 print:hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto mt-4 md:mt-0">
          <div className="w-full sm:w-40">
            ...
            <select value={filterBulan} onChange={(e) => setFilterBulan(e.target.value)} className="input-field w-full p-2.5 sm:p-3 bg-slate-50 lg:bg-white border border-slate-200 rounded-md focus:ring-2 focus:ring-[#82B29A] outline-none text-slate-700 text-sm cursor-pointer transition-all hover:border-[#82B29A]/50">
          ...
          <div className="w-full sm:w-auto flex items-end gap-2">
            <button onClick={() => window.print()} className="w-full sm:w-auto bg-[#88A4C4] hover:bg-[#7590B0] text-white font-bold py-2.5 px-4 rounded-md transition-all shadow-md flex justify-center items-center gap-2 text-sm sm:text-base whitespace-nowrap">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
              <span>Unduh Grafik</span>
            </button>
            <button onClick={() => generatePdfOverview(filteredTasks, filterBulan)} className="w-full sm:w-auto bg-[#F4A261] hover:bg-yellow-500 text-white font-bold py-2.5 px-4 rounded-md transition-all shadow-md flex justify-center items-center gap-2 text-sm sm:text-base whitespace-nowrap">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
          ...
          <p className="text-3xl font-extrabold mt-1 text-[#82B29A]">{wfoCount}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Work From Anywhere</p>
          <p className="text-3xl font-extrabold mt-1 text-[#F4A261]">{wfaCount}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
          ...
          <div className="flex justify-center gap-4 text-xs font-semibold mt-2">
            <div className="flex items-center gap-1.5"><div className="w-3 h-1 bg-[#82B29A]"></div>WFO</div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-1 bg-[#F4A261]"></div>WFA</div>
          </div>
        </div>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={skpData} margin={{top: 0, right: 30, left: 0, bottom: 0}}>
                ...
                <Tooltip cursor={{fill: '#f8fafc'}} />
                <Bar dataKey="Jumlah" fill="#F4A261" barSize={20} radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
                  ...
                  skpData.map(skp => (
                    <tr key={skp.name} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold" style={{ backgroundColor: "rgba(21,134,132,0.1)", color: "#82B29A" }}>
                          {skp.Jumlah}
                        </span>
                      </td>
                    </tr>
              ...
              {skpData.length === 0 ? (
                ...
                skpData.map(skp => (
                  <div key={skp.name} className="bg-slate-50 border border-slate-200 rounded-lg p-3.5 flex justify-between items-center gap-3 shadow-sm">
                    ...
                    <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-[10px] font-bold shrink-0" style={{ backgroundColor: "rgba(21,134,132,0.1)", color: "#82B29A" }}>
                      {skp.Jumlah} kegiatan
                    </span>
                  </div>
                ))
